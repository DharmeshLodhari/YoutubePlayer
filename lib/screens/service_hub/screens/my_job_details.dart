...
import 'package:Slydo/screens/service_hub/models/user_review.dart';
...
class _MyJobsDetailsState extends State<MyJobsDetails> {
  ...
  void getMyJob() async {
    if (!isLoading) {
      ...
      final result = await ServiceHubAuthService().retreiveJob(jobId: jobId);
  ...
  void deleteJob() async {
    final bool done = await ServiceHubAuthService().deleteMyJob(jobId);
    if (done) {
