...
class OrderTrackerStepper extends StatefulWidget {
...
class _OrderTrackerStepperState extends State<OrderTrackerStepper>
  ...
  @override
  void didUpdateWidget(OrderTrackerStepper oldWidget) {
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    ...
    switch (state) {
      ...
      case StepState.error:
        return Text('x', style: _kStepErrorStyle);
    }
  }
  ...
  Widget _buildAnotherCircle(int index, bool oldState) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 8.0),
      width: _kStepSize,
      height: _kStepSize,
  ...
  Widget _buildPendingCircle(int index, bool oldState) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 8.0),
      width: _kStepSize,
      height: _kStepSize,
      child: AnimatedContainer(
        ...
        child: Center(
          child: _buildCircleChild(index,
              oldState && widget.steps[index].state == StepState.editing),
        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      ...
      if (widget.steps[index].state == StepState.editing) {
        return _buildPendingCircle(index, false);
      } else if (widget.steps[index].state != StepState.error) {
        return _buildCircle(index, false);
      } else {
        return _buildAnotherCircle(index, false);
      }
    }
  }
